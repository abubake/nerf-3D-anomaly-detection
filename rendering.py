...
def rendering_uncertainty(model, rays_o, rays_d, tn, tf, nb_bins=100, device='cpu', white_bckgr=True):
    '''
    Rendering function considering new parameters.
    '''
    ...
    colors, density, _ = model.intersect(x.reshape(-1, 3), rays_d.expand(x.shape[0], x.shape[1], 3).transpose(0, 1).reshape(-1, 3))
